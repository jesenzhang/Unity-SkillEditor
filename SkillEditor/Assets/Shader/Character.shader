...
Shader "Instanced2/Character" {
    Properties {
		...
		_buse("NiGuangBuSe", Color) = (0.5,0.5,0.5,1)
        _edgeIntensity ("BianYuanLiangDu", Range(0, 2)) = 1
        _bianse ("FuZhuangBianSe", Color) = (1,0,0,1)
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
        ...
        Pass {
			...
			fixed3 ComputeLight(fixed3 normalDirection, fixed3 lightColor,fixed3 lightDirection, fixed3 normalDir,
				    fixed3 viewDirection,fixed specPow, fixed attenuation, fixed3 halfDirection,
				    fixed3 giIndirect,fixed4 _node_SelfIllum_var, fixed3 attenColor,fixed2 uv0,bool isSkin, bool hasInDirectLight, bool hasEmissive)
			{
				fixed Pi = 3.141592654;
				fixed InvPi = 0.31830988618;
				fixed NdotL = max(0, dot(normalDirection, lightDirection));
				//fixed node_2805 = (_node_SelfIllum_var.g*_edgeIntensity);
				fixed node_2805 = _edgeIntensity;
				fixed node_6128 = (_node_SelfIllum_var.b*pow(1.0 - max(0, dot(normalDir, viewDirection)), _armor_Specular));
				fixed3 specularColor = fixed3(node_6128, node_6128, node_6128);
				...
				fixed3 indirectSpecular = (giIndirect + fixed3(node_2805, node_2805, node_2805))*specularColor;
				...
				fixed para = _armor_Intensity;
				if (isSkin)
				{
					para = 1;
				}
				fixed node_6374 = pow(1.0 - max(0, dot(normalDir, viewDirection)), para);
				fixed3 backLight = max(0.0, -NdotL) * (_buse.rgb*node_6374);
				...
				fixed3 diffuseColor = _main_tex_var.rgb * node_6374  + _node_SelfIllum_var.r;
				...
				fixed3 returnColor = diffuse + specular;
				if (hasEmissive)
				{
					returnColor += emissive;
				}
				
				if (_bianse.x>0.01 && _bianse.y>0.01 && _bianse.z>0.01 &&
					_node_SelfIllum_var.b > 0.01 && _node_SelfIllum_var.b < 0.99)
				{
					fixed3 colorFilter_HSV = RGBConvertToHSV(_bianse);
...
/// Final Color:
				...
				fixed3 finalGlobalColor = ComputeLight(normalDirection, lightColor,lightDirection, i.normalDir,
				...
				fixed4 finalRGBA = finalGlobalRGBA;
